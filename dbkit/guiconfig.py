r"""Tiny JSON-backed config for DBExplorer.

Stores the chosen theme ("light"/"dark") and a short list of recently used
connection-profile names.  On Windows the file lives at
``%LOCALAPPDATA%\DBExplorer\config.json``; elsewhere it falls back to
``~/.dbexplorer/config.json`` -- the SAME directory ``conn.py`` uses for the
connection profiles themselves.  Every function is defensive: a corrupt or
...
def config_dir():
    r"""Directory that holds the config/profile files (created on demand).

    ``%LOCALAPPDATA%\DBExplorer`` on Windows, ``~/.dbexplorer`` otherwise.
    """
...
def add_recent(name):
    """Push a profile *name* to the front of the recent list."""
    if not name:
        return
    cfg = load()
